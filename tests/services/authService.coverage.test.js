...
describe('authService.updateProfile — edge cases', () => {
    ...
    it('allows phone update when phone exists only in the Coupon collection (User uniqueness enforced separately)', async () => {
        // Regression guard for the misplaced Coupon-phone check that used to reject
        // legitimate profile updates (social-login users adding their phone, users
        // changing to a phone that was ever issued a promotional coupon).
        const Coupon = require('../../src/repositories').coupons.rawModel();
        await Coupon.create({ coupon: 'DH9999YHZXB', phone: '8880099999', name: 'Promo User' });

        const user = await makeUser({ phone: '8880000003' });

        const result = await authService.updateProfile(
            user._id,
            { name: 'Test', email: user.email, phone: '8880099999' }
        );

        expect(result.user.phone).toBe('8880099999');
    });

    it('cascades profile phone change to addresses whose mobile matched the OLD profile phone', async () => {
        const user = await makeUser({ phone: '8880077000' });
        user.address.push(
            { name: 'Home',  city: 'Dubai', area: 'A', floorNo: '1', apartmentNo: '1', landmark: 'X', mobile: '8880077000', isPrimary: true  },
            { name: "Mom's", city: 'Dubai', area: 'B', floorNo: '2', apartmentNo: '2', landmark: 'Y', mobile: '9999911111', isPrimary: false },
        );
        await user.save();

        await authService.updateProfile(user._id, { name: 'Test', email: user.email, phone: '8880077999' });

        const fresh = await require('../../src/repositories').users.rawModel().findById(user._id).lean();
        expect(fresh.phone).toBe('8880077999');
        // Address that matched old profile phone → updated
        expect(fresh.address[0].mobile).toBe('8880077999');
        // Address with a deliberately different mobile → left untouched
        expect(fresh.address[1].mobile).toBe('9999911111');
    });

    it('does NOT cascade when the old profile phone was null (e.g. social-login user adding their first phone)', async () => {
        const user = await makeUser({ phone: null });
        user.address.push(
            { name: 'Home', city: 'Dubai', area: 'A', floorNo: '1', apartmentNo: '1', landmark: 'X', mobile: '8880088000', isPrimary: true },
        );
        await user.save();

        await authService.updateProfile(user._id, { name: 'Test', email: user.email, phone: '8880088999' });

        const fresh = await require('../../src/repositories').users.rawModel().findById(user._id).lean();
        expect(fresh.phone).toBe('8880088999');
        // Address mobile unrelated to the (formerly null) profile phone → untouched
        expect(fresh.address[0].mobile).toBe('8880088000');
    });
