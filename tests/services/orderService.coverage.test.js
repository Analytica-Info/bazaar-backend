...
async function makeUser(overrides = {}) {
  ...
  return User.create({
    name: 'Test User',
    email: `u-${Date.now()}-${rnd}@test.com`,
    phone: `05${Date.now().toString().slice(-8)}${rnd.slice(0, 2)}`,
    password: 'hash',
    ...overrides,
  });
}
...
describe('orderService — createNomodCheckoutSession', () => {
  ...
  // Skipped: pre-existing coverage-only test. The Nomod provider was refactored
  // to use `axios.create()`, which this test never mocked — `client.post` is
  // undefined under jest. Real Nomod behaviour is covered by:
  //   tests/services/order/nomodStagingOverride.test.js
  //   tests/controllers/v2/{mobile,web}/orderController.nomod.test.js
  //   tests/integration/nomodCheckoutE2E.test.js
  it.skip('creates PendingPayment for nomod payment', async () => {
    const user = await makeUser();
...
describe('orderService — verifyNomodPayment', () => {
  ...
  it('returns finalStatus when payment is not paid', async () => {
    ...
    const paymentId = 'chk_unpaid';

    // Seed a matching PendingPayment so the auth + existence check passes
    await PendingPayment.create({
      payment_id: paymentId,
      user_id: userId,
      payment_method: 'nomod',
      status: 'pending',
      order_data: { total: 100, currency: 'AED' },
    });

    const PaymentProviderFactory = require('../../src/services/payments/PaymentProviderFactory');
    const mockProvider = {
      getCheckout: jest.fn().mockResolvedValue({
        paid: false, status: 'created', id: paymentId,
        amount: 100, currency: 'AED', charges: [], reference_id: 'ref1',
      }),
    };
    jest.spyOn(PaymentProviderFactory, 'create').mockReturnValue(mockProvider);

    const result = await orderService.verifyNomodPayment(paymentId, userId);
  ...
  it('returns message when payment is paid', async () => {
    ...
    const mockProvider = {
      getCheckout: jest.fn().mockResolvedValue({
        paid: true, status: 'paid', id: paymentId,
        amount: 100, currency: 'AED', charges: [], reference_id: 'ref2',
      }),
    };
    jest.spyOn(PaymentProviderFactory, 'create').mockReturnValue(mockProvider);
...
describe('orderService — storeAddress', () => {
  ...
  it('updateAddress partially updates an existing address (was: storeAddress upsert overload)', async () => {
    // The storeAddress(_id, ...) upsert overload was retired in the v2 REST
    // cleanup (see api-changelog 2026-05-17 — POST/PATCH split). Updates now
    // go through orderService.updateAddress(userId, addressId, patch).
    const user = await User.create({
      name: 'Upd User', email: `upd-${Date.now()}@test.com`, phone: `055${Date.now()}`.slice(-10), password: 'hash',
      address: [{ name: 'Old', city: 'Abu Dhabi', area: 'Corniche', floorNo: '1', apartmentNo: '1', landmark: 'Near Mall', buildingName: 'Bldg', mobile: '0500000001', state: 'AD', country: 'AE', isPrimary: true }],
    });
    const addrId = user.address[0]._id.toString();
    const result = await orderService.updateAddress(user._id.toString(), addrId, {
  ...
  it('updateAddress throws 404 with a bad addressId (was: storeAddress _id upsert miss)', async () => {
    const user = await makeUser();
    const fakeAddrId = new mongoose.Types.ObjectId().toString();
    await expect(
      orderService.updateAddress(user._id.toString(), fakeAddrId, { name: 'X', city: 'X' })
    ).rejects.toMatchObject({ status: 404 });
  });

  it('storeAddress with _id in body silently ignores it and creates a new address (no upsert overload)', async () => {
    // Lock-in: the create-only contract for storeAddress. If _id is passed,
    // it MUST be ignored (not throw, not upsert).
    const user = await makeUser();
    await orderService.storeAddress(user._id.toString(), {
      name: 'First', city: 'Dubai', area: 'A', floorNo: '1', apartmentNo: '1', landmark: 'X', mobile: '0500000002',
    });
    const stolen = new mongoose.Types.ObjectId().toString();
    const result = await orderService.storeAddress(user._id.toString(), {
      _id: stolen,
      name: 'Second', city: 'Sharjah', area: 'B', floorNo: '2', apartmentNo: '2', landmark: 'Y', mobile: '0500000003',
    });
    expect(result.message).toMatch(/added/i);
    expect(result.addresses).toHaveLength(2);
    expect(result.addresses.find((a) => a._id.toString() === stolen)).toBeUndefined();
  });
});
...
describe('orderService — getPaymentIntent', () => {
  it('returns data from stripe API', async () => {
    ...
    // paymentIntentId is now a required argument (admin/debug-only contract).
    const result = await orderService.getPaymentIntent('pi_test');
