...
// ── case-insensitivity (V1 path — checkCouponCode) ─────────────────────────

describe("checkCouponCode — case-insensitive matching", () => {
  beforeEach(() => freezeClock());

  it("matches a stored personal coupon regardless of input casing", async () => {
    // Personal coupons are auto-generated uppercase in production.
    await Coupon.create({ coupon: "DH42YHZXB", phone: "0501111111", status: "unused" });

    const lower = await couponService.checkCouponCode("dh42yhzxb", null, {});
    const upper = await couponService.checkCouponCode("DH42YHZXB", null, {});
    const mixed = await couponService.checkCouponCode("Dh42Yhzxb", null, {});
  ...
  it("matches FIRST15 (and applies its 30 AED cap) for any casing", async () => {
    await Coupon.create({ coupon: "FIRST15", status: "unused" });

    for (const variant of ["FIRST15", "first15", "First15", "  first15  "]) {
      const result = await couponService.checkCouponCode(variant, null, {});
      expect(result.success).toBe(true);
      expect(result.capAED).toBe(30);
      expect(result.type).toBe("coupon");
    }
  ...
  it("matches BankPromoCode for any casing (model auto-uppercases)", async () => {
    await BankPromoCode.create({
      code: "ENBD20",
      discountPercent: 20,
      capAED: 50,
      expiryDate: new Date(FROZEN.getTime() + 86400000 * 30),
      active: true,
      singleUsePerCustomer: false,
      allowedBank: "ENBD",
    });

    const lower = await couponService.checkCouponCode("enbd20", null, {});
    const upper = await couponService.checkCouponCode("ENBD20", null, {});
    const mixed = await couponService.checkCouponCode("Enbd20", null, {});

    expect(lower.type).toBe("promo");
    expect(upper.type).toBe("promo");
    expect(mixed.type).toBe("promo");
  });

  it("regex-special characters in user input do not produce a wildcard match", async () => {
    // Defensive: a malicious user typing '.*' or 'first..' must NOT match
    // 'FIRST15' via accidental regex wildcard expansion.
    await Coupon.create({ coupon: "FIRST15", status: "unused" });

    try {
      await couponService.checkCouponCode(".*", null, {});
      fail("Expected 404 'not valid' rejection");
    } catch (err) {
      expect(err.status).toBe(404);
    }

    try {
      await couponService.checkCouponCode("first..", null, {});
      fail("Expected 404 'not valid' rejection");
    } catch (err) {
      expect(err.status).toBe(404);
    }
