...
describe('authV2.optional()', () => {
  ...
  it('treats expired token as anonymous in optional mode', async () => {
    const token = makeExpiredToken({ id: 'uid-001' });
    const req = mockReq({ headers: { authorization: `Bearer ${token}` } });
    const res = mockRes();
    const next = mockNext();
  ...
  it('treats malformed token as anonymous in optional mode', async () => {
    const req = mockReq({ headers: { authorization: 'Bearer not.a.jwt' } });
    const res = mockRes();
    const next = mockNext();
  ...
  it('treats wrong-secret token as anonymous in optional mode', async () => {
    const wrongSecretToken = jwt.sign({ id: 'uid-001' }, 'wrong-secret', { expiresIn: '1h' });
    const req = mockReq({ headers: { authorization: `Bearer ${wrongSecretToken}` } });
    const res = mockRes();
    const next = mockNext();
  ...
  it('treats valid token for deleted user as anonymous in optional mode', async () => {
    mockUserFindById.mockReturnValue(chainSelect(null));
    const token = makeToken({ id: 'uid-ghost' });
    const req = mockReq({ headers: { authorization: `Bearer ${token}` } });
    const res = mockRes();
  ...
  it('returns 403 for blocked user even in optional mode (deliberate exception)', async () => {
    mockUserFindById.mockReturnValue(chainSelect({ ...fakeUser, isBlocked: true }));
    const token = makeToken({ id: 'uid-001' });
    const req = mockReq({ headers: { authorization: `Bearer ${token}` } });
    const res = mockRes();
  ...
  it('surfaces 500 on unexpected DB error even in optional mode', async () => {
    mockUserFindById.mockReturnValue({
      select: jest.fn().mockReturnValue({
        lean: jest.fn().mockRejectedValue(new Error('DB exploded')),
      }),
