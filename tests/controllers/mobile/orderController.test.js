...
describe("paymentIntent", () => {
  // The route now requires ?paymentIntentId — admin/debug only (BUG-058 hardening).
  it("200 on success", async () => {
    orderService.getPaymentIntent.mockResolvedValue({ clientSecret: "pi_1_secret" });
    const req = makeReq({ query: { paymentIntentId: "pi_test_123" } });
    const res = makeRes();
  ...
  it("400 when paymentIntentId is missing (new admin-only contract)", async () => {
    const req = makeReq({ query: {} });
    const res = makeRes();
    await ctrl.paymentIntent(req, res);
    expect(res.status).toHaveBeenCalledWith(400);
