...
describe("POST /v2/wishlist/items", () => {
  ...
  test("200 — accepts both productId and product_id fields", async () => {
    ...
    const res = await request(app).post("/v2/wishlist/items").set(MOBILE)
      .send({ product_id: "p4" });
...
describe("DELETE /v2/wishlist/items/:productId", () => {
  test("200 — removes product and returns updated wishlist", async () => {
    ...
    const res = await request(app).delete("/v2/wishlist/items/p1").set(WEB);
  ...
  test("404 — product not in wishlist", async () => {
    ...
    const res = await request(app).delete("/v2/wishlist/items/bad").set(WEB);
