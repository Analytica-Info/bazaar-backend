...
exports.checkoutSession = async (req, res) => {
    try {
        await logBackendActivity({
            ...
            status: 'success',
            message: `Stripe checkoutSession API hit - user: ${req.user?._id || 'n/a'}, email: ${req.user?.email || req.body?.user_email || 'n/a'}, payment_method: ${req.body?.payment_method || 'n/a'}. Order data: cartData, shippingCost, name, phone, address, state, city, area, floorNo, buildingName, apartmentNo, landmark, currency, discountPercent, discountAmount, couponCode, mobileNumber, user_email, total, sub_total, txnId, paymentStatus, fcmToken`,
        ...
        const fcmToken = req.user?.fcmToken || null;
        ...
        if (payment_method === 'tabby') {
            ...
            if (data.status !== true) {
                return res.status(400).json({ error: `Payment verification failed. Status: ${data.finalStatus || data.status}` });
            }
        }
        ...
        // const subtotalInDollars = amount_subtotal / 100;
        // const totalInDollars = amount_total / 100; 
        ...
        // const discount_amount_long = Number(amount_subtotal) - Number(total);
        ...
        // Commented out push notification code
        // console.log('sendPushNotification', fcmToken);
        // if (fcmToken) {
        //     await sendPushNotification(
        //         fcmToken,
        //         `Order No: ${order.order_id} Placed Successfully`,
        //         `Hi ${name}, your order of AED ${total.toFixed(2)} is confirmed. Expected by ${formattedDeliveryDate}. Thank you for shopping with Bazaar!`,
        //         user_id,
        //         order._id
        //     );
        // } else {
...
exports.checkoutSessionTabby = async (req, res) => {
    try {
        await logBackendActivity({
            ...
            status: 'success',
            message: `Tabby checkoutSessionTabby API hit - user: ${req.user?._id || 'n/a'}, email: ${req.user?.email || req.body?.user_email || 'n/a'}, payment_method: ${req.body?.payment_method || 'n/a'}. Order data: cartData, shippingCost, name, phone, address, state, city, area, floorNo, buildingName, apartmentNo, landmark, currency, discountPercent, discountAmount, couponCode, mobileNumber, user_email, total, sub_total, txnId, paymentStatus, fcmToken`,
        ...
        if (payment_method !== 'tabby') {
            return res.status(400).json({ error: 'This endpoint is only for Tabby payments' });
        }
        ...
        console.error("❌ [Tabby] Error storing order data:", error);
        await logBackendActivity({
            platform: 'Mobile App Backend',
            activity_name: 'Checkout Session Tabby API Hit',
            status: 'failure',
...
exports.verifyTabbyPayment = async (req, res) => {
    try {
        ...
        if (finalStatus === 'CLOSED') {
            return res.status(200).json({ message: `Payment status is ${status}` });
        }
...
exports.storeAddress = async (req, res) => {
    try {
        ...
        const { _id, name, city, area, floorNo, apartmentNo, landmark, state, mobile, buildingName } = req.body;
...
exports.tabbyWebhook = async (req, res) => {
    try {
        ...
        if (Buffer.isBuffer(req.body)) {
            ...
            console.log("📦 req.body already object");
            data = req.body;
        } else {
            console.log("⚠️ Unexpected req.body type:", typeof req.body);
        ...
        console.log("💥 [Tabby Webhook] Caught in catch block");
