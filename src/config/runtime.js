...
const config = Object.freeze({
  ...
  /**
   * Per-rail timeout for home manifest rail fetches (ms).
   */
  timeouts: {
    /** Max time to wait for a single home rail fetch before treating it as an error. */
    homeRail: envInt('TIMEOUT_HOME_RAIL_MS', 1500),
  },
  ...
  /**
   * Payment reconciler — polls Nomod for pending payments whose redirect never fired.
   * Nomod has no webhooks; this background job is the only recovery mechanism.
   *
   * Disabled by default (RECONCILER_ENABLED=false). Flip to true after first deploy
   * confirms the feature flag is wired and ops monitoring is in place.
   */
  reconciler: {
    /** Master on/off switch. Only the literal string 'true' enables it. */
    enabled: process.env.RECONCILER_ENABLED === 'true',

    /** How often to run the reconciler (converted from minutes to ms). */
    intervalMs: envInt('RECONCILER_INTERVAL_MINUTES', 5) * MS_PER_MINUTE,

    /** Only process PendingPayment records created within this many minutes ago. */
    lookbackMinutes: envInt('RECONCILER_LOOKBACK_MINUTES', 60),

    /** Maximum records to process per tick. Keeps cron ticks bounded. */
    batchSize: envInt('RECONCILER_BATCH_SIZE', 50),

    /** Redis key for the distributed mutex. */
    lockKey: 'reconciler:payment:lock:v1',

    /**
     * Redis lock TTL in seconds.
     * Should be a bit less than intervalMs so the lock is always released
     * before the next tick attempts to acquire it.
     */
    lockTtlSeconds: envInt('RECONCILER_LOCK_TTL_SECONDS', 240),
  },
  ...
  /**
   * Feature flags — opt-in switches for in-progress v2 migrations.
   * Default false everywhere; flip to 'true' in staging .env to test.
   * Only the literal string 'true' enables each flag.
   */
  flags: {
    /**
     * Route cart gift logic through the v2 coupon engine instead of the
     * legacy isGift product flag path. Disabled by default until the
     * seed script has run and parity has been verified in staging.
     */
    cartGiftV2Enabled: process.env.CART_GIFT_V2_ENABLED === 'true',
  },
