...
async function authenticate(req, res, next, { required: isRequired = true, role = 'user' } = {}) {
    ...
    try {
        ...
        if (!userData) {
            // Optional auth: token was valid but the underlying account is gone.
            // Treat as anonymous; do NOT 401.
            if (!isRequired) {
                req.user = null;
                return next();
            }
        ...
        if (userData.isBlocked) {
            // Deliberate exception: a blocked user actively presenting their token
            // is NOT silently treated as anonymous, even on optional-auth routes.
        ...
        // Optional auth: any verify failure (expired, malformed, wrong-secret)
        // is treated identically to "no token sent" — anonymous, not 401.
        // This includes JsonWebTokenError + TokenExpiredError. Internal/unexpected
        // errors (e.g. DB) still surface as 500 even in optional mode.
        const isJwtError =
            error.name === 'TokenExpiredError' || error.name === 'JsonWebTokenError';
        if (isJwtError && !isRequired) {
            req.user = null;
            return next();
        }
