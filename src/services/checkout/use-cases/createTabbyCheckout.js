...
async function createTabbyCheckout(cartData, userId, metadata) {
  try {
    ...
    // BUG-057: last-resort phone fallback — if neither buyer.phone nor orderData.phone
    // are present, look up the user's primary address (where the field is called `mobile`).
    // Saw this on prod order BZ2026999004DBE where the app sent both phone fields empty.
    let resolvedPhone = payment.buyer.phone || phone || '';
    if (!resolvedPhone && userId) {
      const User = repositories.users.rawModel();
      const u = await User.findById(userId).select('phone mobile address').lean();
      if (u) {
        const primary = (Array.isArray(u.address) && u.address.find(a => a.isPrimary))
          || (Array.isArray(u.address) && u.address[0]) || {};
        resolvedPhone = primary.phone || primary.mobile || u.phone || u.mobile || '';
      }
    }
    ...
    payment.meta = {
      ...(payment.meta || {}),
      name: String(name), phone: String(resolvedPhone || phone || ''), address: String(address),
    ...
    const requestBody = {
      payment: {
        ...
        buyer: {
          ...
          // BUG-057: use the resolved phone chain (buyer.phone → orderData.phone → user.address.mobile)
          // so we never send an empty phone to Tabby when we can compute one server-side.
