...
module.exports = async function createTabbySession(userId, bodyData) {
    ...
    const tabbyPayload = {
        ...
        payment: {
            ...
            buyer: {
                email: user.email || '',
                // BUG-057: schema quirk — addresses store the phone as `mobile`,
                // not `phone`. Prefer the checkout-address phone (where customers
                // actually enter it), then user-level. Prod order BZ2026999004DBE
                // had user.phone=null but address[0].mobile="+9715XXXXXXXX".
                phone: primaryAddress.phone || primaryAddress.mobile
