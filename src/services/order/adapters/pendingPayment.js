...
const { applyAutoCoupons } = require('../domain/applyAutoCoupons');
...
async function processPendingPayment(paymentId, payment) {
    try {
        ...
        if (!pendingPayment) {
            // Either the record doesn't exist OR another caller already moved it
            // out of 'pending'. Either way, this caller has nothing to do.
            logger.debug({ paymentId }, '[processPendingPayment] no pending record (already-claimed or missing)');
            return;
        }
        ...
        try {
            const couponResults = await applyAutoCoupons({ order, cart: cartDataValue, user_id, phone: orderData.phone });
            if (couponResults.length > 0) {
                logger.info({ order_id: order._id.toString(), coupon_count: couponResults.length }, 'auto-coupons applied to order');
            }
        } catch (err) {
            logger.error({ err, order_id: order._id.toString() }, 'applyAutoCoupons failed — order continues without coupon redemption');
        }
        ...
        // Symmetric finalization: only update to 'completed' if we still own the record
        // (status is still 'processing' — guards against bizarre concurrent edge cases).
        await PendingPayment.findOneAndUpdate(
            { payment_id: paymentId, status: 'processing' },
        ...
        // Mark as failed only if we currently own the processing slot (not if already completed/cancelled).
        await PendingPayment.findOneAndUpdate(
            { payment_id: paymentId, status: 'processing' },
            { $set: { status: 'failed' } }
        );
