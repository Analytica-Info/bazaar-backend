...
const { resolveAutoGift, nextGiftThreshold } = require('../domain/autoCoupons');
...
async function getCart(userId, options = {}) {
  ...
  if (!cart) {
    if (includeGiftLogic) {
      const useV2Gift = runtime.flags.cartGiftV2Enabled === true;
      if (useV2Gift) {
        const hint = await nextGiftThreshold({ user_id: userId });
        return {
          cartCount: 0,
          cart: [],
          cartSubtotal: 0,
          giftEligible: false,
          giftAdded: false,
          giftProductInStock: false,
          promoMessage: hint
            ? `Add more items to your cart to reach AED ${hint.threshold} and become eligible for a free gift.`
            : null,
        };
      }
  ...
  // ── V2 gift path (feature-flagged) ────────────────────────────────────────
  const useV2Gift = runtime.flags.cartGiftV2Enabled === true;
  if (useV2Gift) {
    const autoGift = await resolveAutoGift({ user_id: userId, cart: { subtotal: cartSubtotal } });

    if (autoGift) {
      const { coupon } = autoGift;
      const giftProductIdStr = coupon.reward?.gift_product_id || '';
      const giftProductName = coupon.reward?.gift_product_name || 'Gift';
      const giftVariantId = coupon.reward?.gift_variant_id || null;

      // Check if the gift is already in the cart as a paid item
      let giftAlreadyInCart = false;
      let v2GiftMarkedCount = 0;

      const cartWithGiftFlagV2 = enrichedItems.map((item) => {
        const isGiftProduct = item.productIdStr === giftProductIdStr;
        let isGiftWithPurchase = false;
        let displayPrice = Number(item.variantPrice || 0);

        if (isGiftProduct) {
          v2GiftMarkedCount += 1;
          if (v2GiftMarkedCount === 1) {
      ...
      let v2GiftAdded = false;
      let v2PromoMessage = `Thank you for shopping with us. As your order qualifies, you will receive ${giftProductName} as a gift.`;

      if (!giftAlreadyInCart) {
        // Append the virtual gift line
        v2GiftAdded = true;
        cartWithGiftFlagV2.push({
          product: giftProductIdStr,
          quantity: 1,
          product_type_id: null,
          image: '',
          name: giftProductName,
          originalPrice: '0',
          productId: giftProductIdStr,
          totalAvailableQty: '1',
          variantId: giftVariantId || giftProductIdStr,
          variantName: 'Default',
          variantPrice: '0',
          isGiftWithPurchase: true,
          price: '0',
        ...
        v2GiftAdded = cartWithGiftFlagV2.some((i) => i.isGiftWithPurchase);
      }

      return {
        cartCount: cartWithGiftFlagV2.length,
        cart: cartWithGiftFlagV2,
        cartSubtotal: Math.round(cartSubtotal * 100) / 100,
        giftEligible: true,
        giftAdded: v2GiftAdded,
        giftProductInStock: true,
        promoMessage: v2PromoMessage,
      };
    }

    // No qualifying gift — build hint message if subtotal is below next threshold
    const hint = await nextGiftThreshold({ user_id: userId, cart: { subtotal: cartSubtotal } });
    const cleanItems = enrichedItems.map(({ unitPrice, itemSubtotal, productIdStr, ...rest }) => ({
      ...rest,
      isGiftWithPurchase: false,
      price: String(Number(rest.variantPrice || 0)),
    }));

    return {
      cartCount: cleanItems.length,
      cart: cleanItems,
      cartSubtotal: Math.round(cartSubtotal * 100) / 100,
      giftEligible: false,
      giftAdded: false,
