...
// ==========================================
// V2 UNIFIED API ROUTES (BFF — mounted unconditionally; v1 routes coexist)
// ==========================================
...
// Wave 1 v2 REST cleanup — login subpaths now nested under /auth/login/* and /auth/refresh.
app.use("/v2/auth/login/google", authLimiter);
app.use("/v2/auth/login/apple", authLimiter);
app.use("/v2/auth/refresh", authLimiter);
...
// Wave 1 v2 REST cleanup — password flow nested under /auth/password/*; recovery flow under /auth/recovery/*.
app.use("/v2/auth/password/forgot", passwordResetLimiter);
app.use("/v2/auth/password/reset", passwordResetLimiter);
app.use("/v2/auth/password/verify-code", passwordResetLimiter);
app.use("/v2/auth/recovery/resend", passwordResetLimiter);
app.use("/v2/auth/recovery/verify", passwordResetLimiter);
...
// Stripe webhook — MUST use raw body parser BEFORE express.json() so that
// stripe.webhooks.constructEvent() can verify the payload signature.
app.post(
  "/api/webhooks/stripe",
  express.raw({ type: "application/json" }),
  stripeWebhookController.handleStripe
);
...
const connectAndRun = async () => {
  ...
  // --- V2 Unified API (mounted unconditionally; v1 routes above coexist) ---
  // The previous V2_ENABLED env-flag gate was removed once v2 hardening landed
  // and the URL surface stabilised — keeping it created dev/prod drift and
  // produced confusing 404s during client integration. V1 routes (/api/*,
  // /check-coupon, /create-coupon, /coupon, etc.) are untouched, so existing
  // clients keep working regardless of whether anything hits /v2/*.
