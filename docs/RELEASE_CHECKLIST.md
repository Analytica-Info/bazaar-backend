...
- [ ] Confirm `NOMOD_ENABLED=true` is set if the feature flag gate is used
...
- If Nomod itself needs to be disabled: set `NOMOD_ENABLED=false` and `PAYMENT_PROVIDER=stripe`, restart
