...
- **Source:** Critical-flows audit (2026-05-01, docs/archive/2026-q2-v2-migration/CRITICAL-FLOWS-AUDIT.md, Flow 2); fixed in Tier-1 fix PR.
...
- **Reference template:** Commit `1acb448` solved the equivalent issue for **Tabby** by adding `POST /api/order/create-tabby-session`. The Stripe fix should follow the same pattern: backend builds the Stripe payload from server-trusted user/order data, calls Stripe with `STRIPE_SECRET_KEY` (server-only), and returns `{ paymentIntentId, clientSecret, ephemeralKey }` for the mobile SDK to consume. See `src/services/order/use-cases/createTabbySession.js` for the structural template.
- **Effort estimate (2026-05-07):** 3-5 working days end-to-end across two repos.
  - Backend (~1.5 days): extend `initStripePayment.js` to also create a `PendingPayment` record (BUG-058 prerequisite); make `processPendingPayment` atomic via `findOneAndUpdate` (BUG-059 prerequisite); add tests covering happy path, race window, missing PendingPayment, signature failure.
  - Mobile (~2-3 days): drop `STRIPE_SECRET_KEY` from `dotenv`; remove the direct `api.stripe.com/v1/{customers,ephemeral_keys,payment_intents}` REST calls; integrate `flutter_stripe` SDK to consume `clientSecret` + `customerId` + `ephemeralKey` from `/stripe/init`; present payment sheet; on success call backend's verify endpoint; regression-test on real iOS + Android builds with sandbox keys.
  - Ops (~1 hour + deploy window): rotate the leaked Stripe live key. Generate new key in Stripe Dashboard → push to backend prod env → restart backend → ship mobile release. Old key stays valid until manually revoked; revoke only after mobile adoption ≥80% (safety against laggards on the App Store / Play Store long tail).
- **Cross-references:** depends on BUG-058 (init must create PendingPayment) and BUG-059 (processPendingPayment must be atomic). Both filed below.
- **Source:** Critical-flows audit (2026-05-01); Tabby variant fixed `2026-05-06` (`1acb448`); Stripe variant still OPEN.
...
### BUG-058 — `/api/order/stripe/init` does not create a PendingPayment record (Stripe webhook handler is half-wired)
- **Severity:** MEDIUM (architectural; webhook is deploy-safe but inert until this lands)
- **Files:** `src/services/order/use-cases/initStripePayment.js`, `src/services/order/use-cases/handleStripeWebhook.js:51-68`
- **Status:** **FIXED** (wave 3, commit `7b999e4`)
- **Fix applied:** `initStripePayment.js` now accepts an optional third `orderData` argument. When supplied (new mobile call pattern), it writes a `PendingPayment({ payment_id: paymentIntent.id, status: 'pending', user_id, order_data })` immediately after creating the Stripe PaymentIntent. Two-argument callers (existing usage) are unaffected — no PendingPayment is written and behavior is identical to the pre-fix state. The webhook handler now finds the record and completes the order correctly.
- **Symptom (historical):** `handleStripeWebhook` looks up a `PendingPayment` doc by `payment_id` and bails if none exists. `initStripePayment` (the only path that should create the matching PendingPayment) only created Stripe-side objects and returned them — it never wrote a PendingPayment doc to MongoDB. As a result, every `payment_intent.succeeded` webhook short-circuited at Layer 2 with `{ skipped: 'no-pending-record' }` and the webhook never created orders.
- **Impact today:** None (webhook is a deploy-safe no-op; old mobile clients use a different code path). The webhook provides ZERO safety-net benefit until this is fixed — i.e. it doesn't recover orders when mobile's success callback fails.
- **Impact once mobile fixes BUG-045 and starts calling `/stripe/init`:** Webhook still won't fire because PendingPayment is never written. The architectural intent (crash-safe order creation via webhook) is unrealized until this is fixed.
- **Recommended fix:** In `initStripePayment.js`, after creating the PaymentIntent, also create a `PendingPayment({ payment_id: paymentIntent.id, status: 'pending', user_id: userId, order_data: { /* cart, addresses, totals from request */ } })`. Reference: the Tabby flow at `src/services/order/use-cases/createTabbyCheckoutSession.js` does this correctly via `await PendingPayment.create({ payment_id, status: 'pending', ... })` — match that pattern.
- **Effort:** half day (write the PendingPayment, add a test that the webhook now finds it).
- **Source:** Webhook dedup verification (2026-05-07).

### BUG-059 — `processPendingPayment` uses non-atomic read-then-save status flip
- **Severity:** MEDIUM (race condition; only manifests once BUG-058 lands and concurrent callers exist)
- **File:** `src/services/order/adapters/pendingPayment.js:32-45`
- **Status:** **FIXED** (wave 3, commit `7b999e4`)
- **Fix applied:** `processPendingPayment` now uses atomic `findOneAndUpdate` with `{ status: 'pending' }` filter for all three state transitions (`pending → processing`, `processing → completed`, `processing → failed`). Only the caller that wins the compare-and-swap (CAS) proceeds; concurrent callers receive `null` and abort idempotently. Lock-in concurrency test in `tests/integration/processPendingPayment.concurrency.test.js` and E2E proof in `tests/integration/nomodCheckoutE2E.test.js` (Flow C).
- **Symptom (historical):** The pattern was:
  ```
  const pendingPayment = await PendingPayment.findOne({ payment_id, status: 'pending' });
  if (!pendingPayment) return;
  pendingPayment.status = 'processing';
  await pendingPayment.save();
  ```
  Two callers could both read `status: 'pending'`, both flip to `'processing'` in memory, both save (the second save just overwrites with the same value), both proceed to create orders. Classic check-then-act race.
- **Source:** Webhook dedup verification (2026-05-07).

---

### BUG-061 — Unauthenticated banner CRUD writes on mobile router

## BUG-061: Unauthenticated banner CRUD writes on mobile router

**Severity:** HIGH (security)
**Status:** OPEN
**Component:** src/routes/mobile/bannerImages.js
**Discovered:** 2026-05-14

The mobile-mount banner router exposes POST /banner, PUT /banner/:id, and
DELETE /banner/:id with NO auth middleware. Any unauthenticated request
can create/update/delete banners on the customer mobile API surface.

The ecommerce-mount equivalents at src/routes/ecommerce/bannerImages.js
have the same shape but should sit behind admin-only auth in the
admin-routes mount; verify whether THIS mount is the actually-exposed
one (it depends on which router is registered in src/server.js).

Recommended fix: gate all three write routes behind admin auth middleware;
or remove the mobile mount entirely if banner writes are admin-only.

---

### BUG-060 — Nomod webhook absence; polling reconciler is the substitute
- **Severity:** MEDIUM
- **File:** `src/services/payments/recovery/pollingReconciler.js`
- **Status:** **FIXED-VIA-MITIGATION** (wave 4, commit `fd595d4`)
- **Symptom:** Nomod has no webhooks per their official API documentation (`docs/vendor/nomod/REFERENCE.md`). The only payment-completion signal available to the backend is either (a) the mobile WebView intercepting the redirect URL and calling `POST /api/verify-nomod-payment`, or (b) a background polling job querying Nomod's `GET /v1/checkout/:id`. Without a recovery mechanism, users who close the app immediately after paying — before the redirect fires — lose their order permanently even though Nomod has collected the money.
- **Mitigation:** An opt-in polling reconciler (`pollingReconciler.js`) runs every 5 minutes via cron. On each tick it:
  1. Acquires a distributed lock (Redis-backed with in-process fallback) to prevent concurrent instances.
  2. Queries MongoDB for all PendingPayments in `status: 'pending'` created within the last `RECONCILER_LOOKBACK_MINUTES` (default 60).
  3. For each record, calls `provider.queryPaymentState(paymentId)` to get the current Nomod status.
  4. Routes to `processPendingPayment(paymentId)` for paid sessions, or marks the record `cancelled`/`expired` for terminal-failure sessions.
  5. Uses the same atomic CAS (`findOneAndUpdate { status: 'pending' }`) as the verify path — concurrent reconciler + verify calls cannot produce duplicate orders.
- **Deploy gate:** `RECONCILER_ENABLED=false` by default. Must be explicitly enabled in production after a staging soak. See `docs/RELEASE_CHECKLIST.md` "Nomod-as-primary deploy gates" section.
- **Cross-references:**
  - Design doc: `docs/payments/PAYMENT-FLOW-DESIGN.md` §1 (webhook absence rationale)
  - Gap analysis: `docs/vendor/nomod/ANALYSIS.md`
  - Wave 4 tests: `tests/integration/pollingReconciler.test.js` (27 scenarios)
  - E2E proof of crash recovery: `tests/integration/nomodCheckoutE2E.test.js` (Flow B)
- **Source:** Nomod integration architecture review (wave 4, 2026-05-01).
